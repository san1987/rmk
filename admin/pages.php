...
if ($newpage) {   mq("INSERT INTO  pages(`name`, `title`) VALUES ('$newpage_name', 'Страница ".time()."') ");	}


if ($delpage)	mq("DELETE FROM   pages WHERE id='".intval($delpage)."'");
...
$sql="SELECT * FROM  pages  ORDER BY `order` ASC, id  DESC";
$r=mq($sql);
...
while ($row=mysql_fetch_assoc($r)) echo "<tr><td>".($href="<a href='$PHP_SELF?p=$p&id=".$row["id"]."'>").$row["title"]."</a> </td><td>".($row["url_name"]==$newpage_name?
			"<font color=red>это вновь созданная вами страница. $href отредактируйте</a> её параметры</font>":"").$row["comment"]." </td><td>/pages/".$row["chpu"]."</td>
