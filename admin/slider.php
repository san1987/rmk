...
if ($del_id)
	$r=mq("DELETE FROM slider WHERE id=".intval($del_id));

if ($edit){

        if($save){
        	mq("UPDATE slider SET
        		`title`='".mres($title)."',
        		`chpu`='".mres($chpu)."',
			...
			if($_FILES['pic']['tmp_name'])
	            imgresize($_FILES['pic']['tmp_name'], "../files/slider/".$edit.".jpg", 800, 412);

        }


		$r=mq("SELECT * FROM slider WHERE id=".intval($edit));
		$row=fetch($r);
        ...
        ?>
     	...
     	<form enctype="multipart/form-data" method=post  style='margin: 10px; border: solid 1px black; padding: 10px'>
				...
				Название<br><input type=text name='title' value='<?=$row["title"]?>'  size=80><br>
		 		Ссылка<br><input type=text name='chpu' value='<?=$row["chpu"]?>'  size=80><br>
		        ...
		        <br>    <br>
		        <?="<img src='../files/slider/".$row["id"].".jpg'>"?>
		        <br>
...
if ($add_new){
	$r=mq("INSERT INTO  slider (`title`) VALUES ('Новый элемент слайдера')");
}
...
foreach ($manuf as $row){
	echo "<tr><td><img src='../files/slider/".$row["id"].".jpg' style=' vertical-align: middle;'></td><td><a href='.?p=$p&edit=".$row["id"]."'>".$row["title"]."</a></td></tr>";
}
