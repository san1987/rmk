...
if ($newpage) {   mq("INSERT INTO  news(`name`, `title`, `cr`) VALUES ('$newpage_name', 'Страница ".time()."', NOW()) ");
	}


if ($delpage)	mq("DELETE FROM   news WHERE id='".intval($delpage)."'");
...
if (isset($id)){
	...
	if ($save){
		...
		if($_FILES['pic']['tmp_name'])
	            imgresize($_FILES['pic']['tmp_name'], "../files/news/".$id.".jpg", 175, 152);

		$newurl_name= do_translit($newurl_name);
	 mq($sql="UPDATE  news SET
		text='".mres($newtext)."',
		title='".mres($newtitle)."',
		chpu='".mres($newchpu)."',
		name='".mres($newname)."',
...
$sql="SELECT * FROM  news  ORDER BY cr,id  DESC";
$r=mq($sql);
...
while ($row=mysql_fetch_assoc($r)) echo "<tr><td><img src='../files/news/".$row["id"].".jpg' style=' vertical-align: middle;'></td><td>".($href="<a href='$PHP_SELF?p=$p&id=".$row["id"]."'>").$row["title"]."</a> </td><td>".($row["url_name"]==$newpage_name?
			"<font color=red>это вновь созданная вами страница. $href отредактируйте</a> её параметры</font>":"").$row["comment"]." </td><td>/news/".$row["chpu"]."</td>
